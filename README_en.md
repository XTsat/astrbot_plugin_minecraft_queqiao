...
| `/mc status [number\|address]` | Everyone | Show server status (incl. online count; player names too when the server exposes them) |
| `/mc list [number\|address]` | Everyone | Show online players (RCON first; falls back to online query when RCON is off; output labels the source used) |
...
`mc status` / `mc list` also support **direct address query**: pass a `host:port` instead (e.g. `mc status 127.0.0.1:25565`, `mc list play.example.com`) and the plugin queries **any** server directly over the Minecraft SLP protocol (no QueQiao, no config needed), returning version, online count, latency, MOTD and online player names (`players.sample`, possibly incomplete); the `list` player names are labelled "online query" as well. When the port is omitted it defaults to `25565`.
