...
- **Web 仪表盘**：内置 AstrBot 插件 Page 页面，打开即可直观查看各服务器在线状态、核心与版本、MOTD、CPU 与内存负载、在线玩家列表（带头像与取数通道标注），并支持快捷广播与指令执行；顶部提供「全部服务器 + 每台服务器」标签切换——全局视图多列展示所有服务器，单服务器视图下统计卡片、服务器卡片与该服专属互通终端一并跟随。互通终端日志**持久化到插件数据目录**（按天分片存储、超期自动清理，网页未打开期间的事件同样记录，跨会话/跨浏览器可恢复），仅点击「清屏」才清除
