...
<p><img alt="version" src="https://img.shields.io/badge/version-v0.5.2-blue"></p>
...
- **Web 仪表盘**：内置 AstrBot 插件 Page 页面，打开即可直观查看各服务器在线状态、核心与版本、MOTD、CPU 与内存负载、在线玩家列表（带头像与取数通道标注），并支持快捷广播与指令执行；顶部提供「全部服务器 + 每台服务器」标签切换——全局视图多列展示所有服务器，单服务器视图下统计卡片、服务器卡片、**性能监控图表**与该服专属互通终端一并跟随。互通终端日志**持久化到插件数据目录**（按天分片存储、超期自动清理，网页未打开期间的事件同样记录，跨会话/跨浏览器可恢复），仅点击「清屏」才清除；终端**增量轮询自动追加**新记录（4 秒），双向互通同流展示——游戏事件（💬 聊天 / 🟢 进出 / 💀 死亡等）与 QQ→游戏 转发（📲）无需手动刷新即可实时看到，右上角另有「↻ 刷新」按钮可随时手动重拉。终端默认**自动滚动**（📌 自动滚动按钮可随时暂停/恢复）：新消息自动滚到底部；暂停后手动滚动不会误开，只有滚到**真正底部**才自动恢复跟随。终端日志**跨天合并展示**（最多保留 30 天、每天 300 条/服，超期自动清理），默认只加载**最近 2 天**（今天+昨天，避免一次拉取全量；「⚙ 设置」弹窗可调「互通终端加载天数」，0=全部保留分片，1～30=最近 N 天，偏好浏览器本地持久化），日期变化处自动插入「── MM-DD ──」分割线，昨天/今天/更早的记录一眼可辨。**互通动态同步写入 AstrBot 主日志**——游戏事件与双向转发（QQ→游戏、游戏→QQ）均以 `[插件名][服务器]` 前缀输出 INFO 日志，转发失败/未连接跳过输出 WARNING，无需打开网页即可在控制台追踪消息去向
- **性能监控（TPS / 延迟）**：**默认开启**的**长时间监控分析**——后台定期经 RCON 采集服务器 TPS（1m/5m/15m 三档）与**直连服务器**的 SLP ping 延迟（玩家视角网络延迟，不经鹊桥中转），采样按天分片持久化（保留天数默认 7 天，超期自动清理）；仪表盘上**「性能监控」面板与服务器面板各占半宽、紧贴排列**（服务器面板保持内容自然高度，监控面板经 JS **双向等高**钳制、与服务器面板同高、底部对齐；「服务器实例状态」标题独立通栏一行；全局视图自动回退单列全宽，服务器卡片横向并排不再细长），以 **TPS / 延迟两个切换子页面**展示趋势图（**🔴 实时模式**：连续测试——每 5 秒对当前服务器采样一次并刷新最近 60 秒曲线（10 秒聚合桶），用于观测瞬时波动；以及 1h/6h/24h/7d 范围切换）与统计摘要（TPS 页：平均/最低 TPS；延迟页：平均/最大/P95 延迟），工具栏「⏱ 立即采集」可随时对当前服务器触发一轮采样（不等采集间隔）、「⚙ 设置」里可选择**默认展示页面**（打开仪表盘时自动显示 TPS 或延迟，随设置一起持久化到服务器端），`/mc status` 也会附带最近采样值。**无需任何 WebUI 配置**：监控参数（开关/间隔/实时采集频率/自动刷新间隔/保留/指令/延迟探测域名与端口/默认展示页）在面板「⚙ 设置」调整（弹窗已加宽以便阅读），保存即时生效；**TPS 需服务端支持 tps 指令（经 RCON 执行）；延迟直接连接服务器——须在「⚙ 设置」显式填写公网探测地址（域名 + MC 游戏端口，如 `25565`），留空则不采集延迟**（不回落内网地址，避免把内网互通速度当成玩家延迟；54040 这类鹊桥 Web 端口不是 MC 游戏端口，填了会探测失败）
...
| `/mc status [编号\|地址]` | 全部 | 查看服务器状态（含在线人数；服务器返回名单时也显示玩家名） |
| `/mc list [编号\|地址]` | 全部 | 查看在线玩家列表（优先 RCON，未开 RCON 时用在线查询兜底；回报标注取数方式） |
...
`mc status` / `mc list` 还支持**地址直连**：把参数换成 `host:port`（如 `mc status 127.0.0.1:25565`、`mc list play.example.com`），插件会直接以 Minecraft SLP 协议查询**任意**服务器（不经鹊桥、无需配置），返回版本、在线人数、延迟、MOTD 与在线玩家名（`players.sample`，可能不全）；`list` 的玩家名单同样标注「在线查询」。省略端口时默认 `25565`。
...
> **监控类别**：TPS 与延迟按「长时间监控」对待——定期采样、持久化、可分析，
> 而非每次查询实时抓取。**默认开启、开箱即用**，不带任何 WebUI 配置项
> （不占用 `_conf_schema.json`）；参数（启用开关 / 采集间隔 / 保留天数 /
> TPS 指令）在 **Web 仪表盘「性能监控」面板右上角「⚙ 设置」** 内维护，
> 保存后立即生效并持久化到插件数据目录 `data_dir/monitor/settings.json`。

| 参数 | 默认值 | 说明 |
|------|--------|------|
| 启用开关 | `on` | 关闭后停止采样（历史数据保留至过期自动清理） |
| 采集间隔 | `60` 秒 | 最小 `10` 秒。调小更灵敏但增加服务器与 RCON 负担 |
| 保留天数 | `7` 天 | 超过该天数的历史采样自动清理，调大可支持更长周期分析（如 30 天） |
| TPS 指令 | `auto` | **默认自动**：按直连 SLP ping 识别到的服务端品牌选择指令（Forge → `forge tps`；Fabric/Quilt → `spark tps`；Paper/Spigot 等 Bukkit 系 → `tps`；未识别 → `tps`）。也可固定指定 `tps` / `forge tps` / `spark tps` |
| 延迟探测域名 | 留空 | 直连服务器的**公网域名/地址**（可含端口，如 `mc.example.com:25565`），用于测玩家视角的真实网络延迟；**留空时取插件配置 `ws_url` 的主机名**（正向模式鹊桥与 MC 服务器同机，即服务器内网地址——测到的是内网互通速度，外网玩家看到的值会更高） |
| 延迟探测端口 | `25565` | MC 服务端口；非标端口或服务端禁用 SLP 时，延迟显示不可用 |

**数据来源与限制**：

- **TPS**：经 RCON 通道执行 TPS 指令并解析 `TPS from last 1m, 5m, 15m: 20.0, 20.0, 20.0` 三档值。`auto` 模式首次采样先按默认 `tps` 尝试，随后根据 SLP ping 识别到的服务端品牌**自动切换**为对应指令（如检测到 `forge 1.20.1` 即改用 `forge tps`）。原版端不支持任何 tps 指令，此时 TPS 记录为「不可用」并在仪表盘状态行提示。
- **延迟**：**直连服务器**执行 Minecraft SLP ping（服务端状态协议握手 + 状态请求往返，ms）——与玩家客户端看到的延迟同一测量口径，**不经鹊桥中转**、无需 RCON、无需服务端安装插件。探测目标优先用设置在仪表盘的**延迟探测域名**（测公网玩家视角），留空则取配置 `ws_url` 的主机名 + 端口 25565。服务器关闭 SLP（`enable-status=false`）或地址不可达时延迟未采到，TPS 不受影响。
- **存储位置**：`data_dir/monitor/<server_name>/YYYY-MM-DD.jsonl`，按天分片；重启不丢，超保留期自动清理。
- **超时语义**：TPS 或延迟采样超时（结果未知）时记录「未采到」并跳过，**不会**自动换通道重发，避免重复执行指令。
...
回报会在人数后标注本次取数方式：RCON 命中时区分「鹊桥RCON」（经鹊桥 `send_rcon_command`）与「直连RCON」（鹊桥不可用时回退直连），在线查询（SLP）命中时标「在线查询」。
